package lyzzcw.work.component.common.MJUtils.JudegHu.checkHu;
...
public class Hulib {
	...
	boolean _split(int[] cards, int gui_num, int color, int min, int max,
			boolean chi, ProbabilityItemTable ptbl) {
		int key = 0;
		int num = 0;
	...
	boolean list_probability(int color, int gui_num, int num, int key,
			boolean chi, ProbabilityItemTable ptbl) {
		boolean find = false;
	...
	boolean check_probability_sub(ProbabilityItemTable ptbl, boolean eye,
			int gui_num, int level, int max_level) {
		for (int i = 0; i < ptbl.m_num[level]; ++i) {
			...
			if (level < max_level - 1) {
				int last_gui = gui_num-item.gui_num;
				if (check_probability_sub(ptbl, eye || item.eye, last_gui,
						level + 1, ptbl.array_num)) {
					return true;
				}
				continue;
			}
			...
			/**
			 * 这段的意思是如果前面的几种组合类型都没有将的时候，需要让混的个数加2，然后跟混牌个数比较
			 */
...
//	ProbabilityItem[][] m = new ProbabilityItem[4][5];//当手中的混为7个的时候，会报错
	ProbabilityItem[][] m = new ProbabilityItem[4][7];
