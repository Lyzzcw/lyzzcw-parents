package lyzzcw.work.component.common.http.client;
...
//            logger.info("请求{}接口的参数为{}",url,jsonParam);
            ...
            logger.error("请求{}接口出现异常",e);
            if (reSend > 0) {
                logger.info("请求{}出现异常:{}，进行重发。进行第{}次重发",e);
...
//        logger.info("请求{}接口的响应报文内容为{},本次请求API接口的响应时间为:{}毫秒",url,result,(endTime-startTime));
     ...
     * @throws Exception
     */
    public static String sendPostByForm(String url, Map<String,Object> map,int reSend) {
        //声明返回结果
        String result = "";
        //开始请求API接口时间
        long startTime=System.currentTimeMillis();
        ...
        try {
            // 创建连接
            httpClient = HttpClientFactory.getInstance().getHttpsClient();
            // 设置请求头和报文
            HttpPost httpPost = HttpClientFactory.getInstance().httpPost(url);
            //设置参数
